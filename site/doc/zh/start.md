...
本项目的官方实例是 `https://pair.taoai.site`。电脑必须能打开本机 Herdr。目前支持 macOS 和 Linux，Windows 还不支持。
...
这会下载 `pairfob`、向官方实例 `https://pair.taoai.site` 登记，并装上登录即启动的用户服务。
...
它会优先画出二维码，并保留一串给手输用的码。另一台设备打开 <a href="/pair">pair.taoai.site/pair</a>：
...
1. 打开 <a href="/pair">pair.taoai.site/pair</a>（已配对后会直接进列表）
2. 分享 → 添加到主屏幕
3. 以后从图标打开
