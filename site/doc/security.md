...
`pair.taoai.site` is this project's official relay. It does not run Herdr. Provided the devices and client code have not been tampered with, the session is end-to-end encrypted after pairing, and keys live only on your device and on the computer.
...
On a P2P path, session bytes do not go through `pair.taoai.site`; the relay still only sees the ciphertext used to set up that path. To find a direct path, the browser asks Cloudflare’s public-address lookup, which sees this device’s public address. Pairfob does not run a bypass for firewalls; if a direct path cannot be found, the session stays on the relay.

## Trust in the website's code

The browser's encryption and decryption code comes from `pair.taoai.site`, so you still need to trust the official site and its code publishing channel. If the site operator or someone who controls that channel serves malicious code, it could read and send out keys or plaintext. End-to-end encryption cannot protect a client that has been tampered with.
...
You do not open ports at home and you do not bind Tailscale. The computer connects out to `pair.taoai.site`.
...
| `pair.taoai.site` | Ids needed to route. No session contents |

## Out of scope

- Malicious control of the official website's code or publishing channel
- A fake Pairfob page — use `https://pair.taoai.site`
