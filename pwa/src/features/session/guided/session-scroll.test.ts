...
import { applyPaneRead, livePaneText, selectPane, setFullTerminal } from "../session-store";
...
import { pageScrollLines, sendGuidedTuiScroll } from "./term";
...
afterEach(() => {
  ...
  // Unmount before domain resets: named actions publish immediately and would
  // notify a still-mounted subscriber outside act. The original facade writes
  // only marked dirty, so the order change is behavior-neutral teardown.
  unmountReact();
...
describe("react session scroll rail", () => {
  ...
  test("上一页 / 下一页 write CSI into the PTY, not SendKeys pageup", async () => {
    ...
    attachLiveSession(live({
      ...
      paneRead: async () => ({ text: livePaneText(), hash: "h" }),
    }) as unknown as LiveSession);
  ...
  test("page-pending subscription updates both React rails without DOM patching", async () => {
    ...
    const secondBtn = appRoot().querySelectorAll<HTMLElement>(".scroll-page-up")[1];
    ...
    const reactBtn = appRoot().querySelector(".scroll-page-up") as HTMLElement;
    ...
    expect(appRoot().querySelector("[data-react-session-scroll]")?.getAttribute("aria-busy")).toBe("true");
  ...
  test("unmount cancels rail hold repeats immediately", async () => {
    ...
    const up = appRoot().querySelector(".scroll-up") as HTMLButtonElement;
  ...
  test("keyboard Space fires once without duplicating a pointer click", () => {
    ...
    const [lineUp, pageUp] = [...appRoot().querySelectorAll("button")] as HTMLButtonElement[];
