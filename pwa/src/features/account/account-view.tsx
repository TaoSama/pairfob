...
export function AccountDevicesView({
  username, isAdmin, devices, busy, locked, syncFailed, errorCode, onAddDevice, onUseDevice, onSignOut, onSwitch, onRetry,
}: {
  username: string;
  isAdmin: boolean;
  devices: readonly BoundDevice[];
  busy: boolean;
  /** Signed in, but no vault key here, so synced machines cannot be opened. */
  locked: boolean;
  syncFailed: boolean;
  errorCode: string | null;
  onAddDevice: () => void;
  onUseDevice: (daemonId: string) => void;
  onSignOut: () => void;
  onSwitch: () => void;
  onRetry: () => void;
}) {
  return (
    <div className="page account-page account-devices-page">
      <div className="account-identity">
        <p className="account-who">{t("account.signedInAs", { name: username })}</p>
        {isAdmin ? <span className="pill pill-live">{t("account.roleAdmin")}</span> : null}
      </div>
      <h1 className="prelude-title">{t("account.devices.title")}</h1>
      {errorCode ? <Feedback value={{
        text: t(errorCode === "bad_relay" || errorCode === "timeout" ? "account.error.offline" : "account.error.unknown"),
        tone: "error",
      }} /> : null}
      {syncFailed ? (
        <>
          <Feedback value={{ text: t("account.syncFailed"), tone: "error" }} />
          <Button className="btn btn-small btn-ghost" onClick={onRetry}>{t("retry")}</Button>
        </>
      ) : null}
      {locked ? <Feedback value={{ text: t("account.devices.locked"), tone: "status" }} /> : null}
      {busy ? <Feedback value={{ text: t("account.devices.loading"), tone: "status" }} /> : null}
      {devices.length ? (
        <ul className="account-device-list">
          {devices.map((device) => (
            <li key={device.daemonId} className="account-device">
              <Button className="account-device-open" onClick={() => onUseDevice(device.daemonId)}>
                <span className="account-device-name">{device.label || device.daemonId}</span>
                <span className={`pill ${device.live ? "pill-live" : "pill-idle"}`}>
                  {device.live ? t("account.devices.live") : t("account.devices.offline")}
                </span>
              </Button>
            </li>
          ))}
        ...
        <EmptyState spec={{
          figure: "device",
          title: t("account.devices.emptyTitle"),
          sub: t("account.devices.empty"),
        }} />
      )}
      <Button className="btn btn-primary account-add-device" onClick={onAddDevice}>
        {t("account.addDevice")}
      </Button>
      <div className="account-session-controls">
        <Button className="btn btn-small btn-ghost account-switch" onClick={onSwitch}>
          {t("account.switchAccount")}
        </Button>
        <Button className="btn btn-small btn-danger account-sign-out" onClick={onSignOut}>
          {t("account.signOut")}
        </Button>
      </div>
    </div>
